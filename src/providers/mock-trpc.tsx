...
// Mock hooks
const createMockHook = (data: any) => () => ({ data, isLoading: false, isError: false, error: null });
const createMockMutation = (onSuccess?: () => void) => () => ({
  mutate: (variables: any) => {
    if (onSuccess) onSuccess();
    return { data: true };
  },
  mutateAsync: async (variables: any) => true,
  isLoading: false,
  isError: false,
  error: null,
});
...
const mockClient = {
  branch: {
    list: { useQuery: createMockHook(MOCK_BRANCHES) },
    getProducts: { useQuery: createMockHook(MOCK_PRODUCTS) },
  },
  product: {
    list: { useQuery: createMockHook(MOCK_PRODUCTS) },
    toggleAvailable: { useMutation: createMockMutation() },
  },
} as any;
